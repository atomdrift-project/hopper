...
# that. The HTML dashboard is a separate listener (--dashboard-addr, every
# interface by default) with no authentication of its own, so it must never be
# given a tunnel hostname; reach it over the LAN or an SSH forward instead.
#
